...
const LENDERS = [
  ...
  { key: 'avanse_global', remarksKey: 'avanse_global_remarks', name: 'AVANSE GLOBAL' },
  ...
  { key: 'union_bank', remarksKey: 'union_bank_remarks', name: 'UNION' }
];
...
export default function FilesUpdate() {
  const { user } = useAuth();
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const res = await http.get(`/file-updates?${q}`);
      setRows(res.data.files || []);
      setPagination(res.data.pagination || { page: 1, pages: 1, total: 0 });
    } catch (err) {
      console.error('Failed to load file-updates:', err);
      setNotice(err.response?.data?.error || 'Unable to load file update records');
    } finally {
      setLoading(false);
    }
  }, [filter]);
  ...
  const handleSave = async (e) => {
    ...
    setSubmitting(true);
    try {
      await http.patch(`/file-updates/${editItem.id}`, editForm);
      setNotice(`Updated lender statuses for ${editItem.name}`);
      setEditItem(null);
      loadData();
      setTimeout(() => setNotice(''), 3000);
    } catch (err) {
      console.error('Failed to update file-update record:', err);
      alert(err.response?.data?.error || 'Failed to save bank status updates');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <section className="page files-update-container">
      {/* Header Banner */}
      <div className="fu-head">
        <div>
          <span style={{ fontSize: '10px', fontWeight: 900, letterSpacing: '1.5px', color: '#9b78e7' }}>FILES PIPELINE</span>
          <h1>Files Update</h1>
          <p>Manage your collected Files</p>
        </div>
      </div>
      ...
      <div className="fu-filters">
        <label>
          ⌕ <input
            type="text"
            placeholder="Search name, phone, email, university or city..."
            value={filter.search}
      ...
      {/* Count Bar */}
      <div className="fu-count">
        <span><b>{pagination.total}</b> files collected</span>
        ...
        <table className="fu-table">
          <thead>
            <tr>
              ...
              {/* Dynamic Lender & Remarks Headers */}
              {LENDERS.map(lender => (
                <React.Fragment key={lender.key}>
                  <th className="bank-header">{lender.name}</th>
                  <th className="remarks-header">REMARKS</th>
                </React.Fragment>
              ))}

              <th style={{ textAlign: 'center' }} title="Edit Lender Application Statuses"><EditIcon /></th>
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan={15 + LENDERS.length * 2} style={{ textAlign: 'center', padding: '40px', color: '#7e8999' }}>
                  Loading files update data...
                </td>
              </tr>
            ) : rows.length === 0 ? (
              <tr>
                <td colSpan={15 + LENDERS.length * 2} style={{ textAlign: 'center', padding: '40px', color: '#7e8999' }}>
                  No file update records found.
                </td>
              </tr>
            ) : (
              rows.map(item => (
                <tr key={item.id}>
                  ...
                  <td>
                    <a href={`tel:${item.phone}`} className="fu-phone">{item.phone}</a>
                  </td>
                  <td>{item.email || '—'}</td>
                  <td>{item.country || '—'}</td>
                  ...
                  {/* 14 Lenders Status & Remarks Columns */}
                  {LENDERS.map(lender => {
                    const st = item[lender.key] || 'NOT MOVED';
                    const rem = item[lender.remarksKey] || '—';
                    return (
                      <React.Fragment key={lender.key}>
                        <td>
                          <span className={`bank-status-pill st-${st.toLowerCase().replace(/\s+/g, '_')}`}>
                            {st}
                          </span>
                        </td>
                        <td style={{ fontSize: '11px', color: '#7e8999' }}>{rem}</td>
                      </React.Fragment>
                    );
                  })}
      ...
      {editItem && (
        <div className="overlay">
          <form className="lead-form" onSubmit={handleSave} style={{ width: 'min(920px, 100%)', background: '#ffffff', color: '#35435a' }}>
            <div className="modal-top" style={{ borderBottom: '1px solid #eef0f4' }}>
              <div>
                <span style={{ fontSize: '10px', fontWeight: 900, letterSpacing: '1.5px', color: '#9b78e7' }}>FILES UPDATE MANAGEMENT</span>
                <h2 style={{ color: '#28364e', margin: '4px 0 0 0' }}>Update Lender Statuses — {editItem.name} ({editItem.phone})</h2>
              </div>
            ...
            <div className="bank-modal-grid">
              {LENDERS.map(lender => (
                <div key={lender.key} className="bank-card-field">
                  <strong>{lender.name}</strong>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                    <select
                      value={editForm[lender.key] || 'NOT MOVED'}
                      onChange={e => setEditForm(prev => ({ ...prev, [lender.key]: e.target.value }))}
                    >
                      {BANK_STATUSES.map(s => (
                    ...
                    <input
                      type="text"
                      placeholder="Remarks / App No..."
                      value={editForm[lender.remarksKey] || ''}
                      onChange={e => setEditForm(prev => ({ ...prev, [lender.remarksKey]: e.target.value }))}
                    />
                  </div>
                </div>
              ))}
            ...
            <div className="modal-footer" style={{ borderTop: '1px solid #eef0f4' }}>
              ...
              <button type="submit" className="primary" disabled={submitting}>
                {submitting ? 'Saving...' : 'Save Bank Updates'}
              </button>
            </div>
          </form>
        </div>
