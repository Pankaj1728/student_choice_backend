...
export default function DisbursementManagement() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const res = await http.get(`/disbursements?${q}`);
      setRows(res.data.disbursements || []);
      setPagination(res.data.pagination || { page: 1, pages: 1, total: 0 });
    } catch (err) {
      console.error('Failed to load disbursements:', err);
      setNotice(err.response?.data?.error || 'Unable to load disbursement records');
    } finally {
      setLoading(false);
    }
  }, [filter]);
  ...
  const openEditModal = (item) => {
    setEditItem(item);
    let dDate = item.disbursed_date ? new Date(item.disbursed_date).toISOString().slice(0, 10) : '';
    setEditForm({ ...item, disbursed_date: dDate });
  };
  ...
  const handleSave = async (e) => {
    ...
    setSubmitting(true);
    try {
      await http.patch(`/disbursements/${editItem.id}`, editForm);
      setNotice(`Updated disbursement details for ${editItem.name}`);
      setEditItem(null);
      loadData();
      setTimeout(() => setNotice(''), 3000);
    } catch (err) {
      console.error('Failed to update disbursement record:', err);
      alert(err.response?.data?.error || 'Failed to save disbursement details');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <section className="page disb-container">
      {/* Header Banner */}
      <div className="disb-head">
        <div>
          <span style={{ fontSize: '10px', fontWeight: 900, letterSpacing: '1.5px', color: '#9b78e7' }}>DISBURSEMENT PIPELINE</span>
          <h1>Disbursement</h1>
          <p>Manage your Disbursed Loans & Tranches</p>
        </div>
      </div>
      ...
      {/* Count Bar */}
      <div className="disb-count">
        <span><b>{pagination.total}</b> disbursement records found</span>
        <span>Page {pagination.page} of {pagination.pages}</span>
      </div>

      {/* Table Container */}
      <div className="disb-table-wrapper">
        <table className="disb-table">
          <thead>
            <tr>
              ...
              <th>CITY</th>
              <th>INTERESTED</th>
              <th className="disb-highlight">LENDER</th>
              <th className="disb-highlight">SANCTION AMOUNT</th>
              <th className="disb-highlight">DISBURSED AMOUNT</th>
              <th className="disb-highlight">TRANCHE NO</th>
              <th className="disb-highlight">DISBURSED DATE</th>
              <th className="disb-highlight">STATUS</th>
              <th className="disb-highlight">REMARKS</th>
              <th style={{ textAlign: 'center' }} title="Edit Disbursement Details"><EditIcon /></th>
            ...
            {loading ? (
              ...
              rows.map(item => (
                <tr key={item.id}>
                  ...
                  <td>
                    <a href={`tel:${item.phone}`} className="disb-phone">{item.phone}</a>
                  </td>
                  <td>{item.email || '—'}</td>
                  <td>{item.country || '—'}</td>
                  ...
                  <td style={{ fontWeight: 700, color: '#15803d' }}>{money(item.disbursed_amount)}</td>
                  <td style={{ fontWeight: 600 }}>{item.tranche_number || '1ST TRANCHE'}</td>
                  <td>{formatDate(item.disbursed_date)}</td>
                  <td>
                    <span className={`disb-badge st-${(item.status || 'DISBURSED').toLowerCase().replace(/\s+/g, '-')}`}>
        ...
        <div className="overlay">
          <form className="lead-form" onSubmit={handleSave} style={{ width: 'min(780px, 100%)', background: '#ffffff', color: '#35435a' }}>
            <div className="modal-top" style={{ borderBottom: '1px solid #eef0f4' }}>
              <div>
                <span style={{ fontSize: '10px', fontWeight: 900, letterSpacing: '1.5px', color: '#9b78e7' }}>DISBURSEMENT MANAGEMENT</span>
                <h2 style={{ color: '#28364e', margin: '4px 0 0 0' }}>Update Disbursement — {editItem.name}</h2>
              </div>
              <button type="button" onClick={() => setEditItem(null)} style={{ background: '#f0f1f5', color: '#333' }}>×</button>
            </div>

            <div className="disb-modal-grid">
              <div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e => setEditForm(prev => ({ ...prev, disbursed_amount: e.target.value }))}
                />
              </div>
              ...
              <div>
                ...
                <select
                  value={editForm.tranche_number || '1ST TRANCHE'}
                  onChange={e => setEditForm(prev => ({ ...prev, tranche_number: e.target.value }))}
                >
                  <option value="1ST TRANCHE">1ST TRANCHE</option>
                  <option value="2ND TRANCHE">2ND TRANCHE</option>
                  <option value="3RD TRANCHE">3RD TRANCHE</option>
                  <option value="FINAL TRANCHE">FINAL TRANCHE</option>
                </select>
              </div>
              ...
              <div>
                ...
                <input
                  type="date"
                  value={editForm.disbursed_date || ''}
                  onChange={e => setEditForm(prev => ({ ...prev, disbursed_date: e.target.value }))}
                />
              </div>
              ...
              <div>
                <label>Status</label>
                <select
                  value={editForm.status || 'DISBURSED'}
            ...
            <div className="modal-footer" style={{ borderTop: '1px solid #eef0f4' }}>
              ...
              <button type="submit" className="primary" disabled={submitting}>
                {submitting ? 'Saving...' : 'Save Disbursement Details'}
              </button>
            </div>
          </form>
        </div>
